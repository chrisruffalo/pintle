...
public class HostFileHandler extends FileSourceHandler {
    ...
    public int process(long listId, ActionList config, StoredSource storedSource) {
        ...
        logger.debugf("loading list %d, source %d, from data path: %s", listId, storedSource.id, dataPath);
        ...
        try (
            ...
            while((line = reader.readLine()) != null) {
                ...
                final StoredLine inserted = StoredLine.createOrGetByListSourceAndHostname(listId, storedSource.id, hostname, address);
                logger.debugf("inserted stored line with id %s", inserted.id);
                loaded++;
            }
        } catch (IOException e) {
        ...
        logger.infof("[list %s] loaded %d items from %s/.../%s [%dms]", config.name(), loaded, host, file, start.until(ZonedDateTime.now(), ChronoUnit.MILLIS));
